...
?> 
    ...
    <body><h1>Forecast Search</h1>
        <div id = "container" class="mydiv1">
...
if(isset($_POST["submit"])) {
     ...
     $lattitude = $xml_obj->result[0]->geometry->location->lat ;
        //Get lattitude
     
     $longitude = $xml_obj->result[0]->geometry->location->lng ;
        //Get Longitude
     /*
     ...
     */   
     ...
     $json_filecontents =  file_get_contents("https://api.forecast.io/forecast/673646bfee45295e9b52590073a229ab/" .$lattitude ."," . $longitude . "?units=" . $unit . "&exclude=flags");
     ...
     if ($_SESSION["degree"] == "fahrenheit"){
         $temperature = number_format($json_obj->{"currently"}->{"temperature"},0,'.', ',') . '&#176F';
     }else{
         $temperature = number_format($json_obj->{"currently"}->{"temperature"},0,'.', ',') . '&#176C';
     }
    ...
    if($unit == 'us'){
        ...
        $ws = 3.6 * $json_obj->{"currently"}->{"windSpeed"} ;
        $wind_speed = number_format(round($ws), 0 , '.', ',') .' km/h';
         $visibility = number_format(round($json_obj->{"currently"}->{"visibility"}), 0 , '.', ',') . ' km';
    }
     
     $dew_point = number_format(round($json_obj->{"currently"}->{"dewPoint"}), 0 , '.', ',');
...
?>
        ...
        <div id="result_container" class="mydiv2">
